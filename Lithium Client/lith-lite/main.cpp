...
BOOL CALLBACK enum_windows_callback(HWND hWnd, LPARAM lparam)
{
    ...
    if (IsWindowVisible(hWnd) && length != 0)
    {
        for (std::string s : bad_titles)
        {
            ...
            {
                std::string url = xors("&hwid=") + lith::info::hwid + xors("&process=") + (std::string)buffer;
                replacestr(url, xors(" "), xors("%20"));
...
| $$      | $$  | $$ /$$| $$  | $$| $$| $$  | $$| $$ | $$ | $$      | $$      | $$  | $$ /$$| $$_____/
| $$$$$$$$| $$  |  $$$$/| $$  | $$| $$|  $$$$$$/| $$ | $$ | $$      | $$$$$$$$| $$  |  $$$$/|  $$$$$$$
|________/|__/   \___/  |__/  |__/|__/ \______/ |__/ |__/ |__/      |________/|__/   \___/   \_______/                                                                                                     
    )") << std::endl;
    ...
    if (detected.size() >= 1)
    {
        ...
        for (std::string s : detected)
            detected_process.append(s + xors(", "));
        std::string url = xors("&hwid=") + lith::info::hwid + xors("&process=") + (std::string)detected_process;
    ...
    std::string drive_name = (std::string)getenv(xors("SystemDrive"));
    drive_name.append(xors("\\"));

    DWORD drive_serial = 0;
    GetVolumeInformationA(drive_name.c_str(), NULL, NULL, &drive_serial, NULL, NULL, NULL, NULL);

    std::ostringstream o;
    o << std::hex << HIWORD(drive_serial) << LOWORD(drive_serial);
    std::string hwid = o.str();

    std::string* hwid_key = new std::string(xors("0dZViYiqIwbJlM4DI4Qk8BCGFtv0GeO7PV1RE9tnXynmiMYJ1i0Gfjzo21VVjT174LaHEF1wax2lYJ8kDQ2wIw6diDfOuBq8Phs4gLlTiMz9BdV4aE1mSLYra730JrRq"));

    hwid = xorr(hwid, hwid_key->c_str(), hwid_key->size());
    delete hwid_key;

    hwid = md5(sha512(hwid));
    lith::info::hwid = hwid;
    ...
    lith::info::stage = 4;
        ...
        std::cout << xors("Authenticated.") << std::endl;
        lith::info::verified = true;
    ...
    while (true) {
        ...
        if (first) {
            ...
            if (!lith::tools::createhandletoprocess(lith::info::pid)) {
                ...
                std::string cmd = xors("/C echo Error - Lithium Lite v") + std::string(lith::info::version) + xors(" & echo. & echo Unable to create a handle to the target process, please contact support. ") + std::string("Stage: " + std::to_string(lith::info::stage)) + xors("& echo. & pause");
    ...
    ::RegisterClassEx(&wc);
    lith::gui::hwnd = ::CreateWindow(wc.lpszClassName, "", WS_POPUP, x / 2 - 167, y / 2 - 89, 335, 178, NULL, NULL, wc.hInstance, NULL);

    if (!CreateDeviceD3D(lith::gui::hwnd)) {
